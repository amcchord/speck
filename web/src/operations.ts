...
export function createOperations(ui: Item) {
  ...
  async function renderSoftware() {
    ...
    content(
      `<div class="page-intro"><div><h2>Template library</h2><p>Reusable installers and scripts for Windows and Linux.</p></div>${button("new-template", icon("plus") + " Create template", true)}</div><div class="template-grid">${library.map((t) => `<article class="template-card"><div class="template-card-top"><span class="template-icon">${icon(t.category === "software" ? "package" : "terminal")}</span>${badge(t.platform)}${t.builtin ? "<small>STARTER</small>" : ""}</div><h2>${esc(t.name)}</h2><p>${esc(t.description || "Reusable fleet script")}</p><div class="template-meta"><span>${t.parameters.length} inputs</span><span>${Math.ceil(t.timeout / 60)} min limit</span><span>v${t.revision}</span></div><div class="toolbar"><button class="primary" data-deploy="${t.id}">Deploy</button><button class="secondary" data-edit-template="${t.id}">${t.builtin ? "Customize" : "Edit"}</button></div></article>`).join("")}</div><div class="section-head"><h2>Recent deployments</h2>${button("software-refresh", "Refresh")}</div><div class="operation-history">${
        batches
  ...
  async function renderAssistant() {
    ...
    content(
      `<div class="assistant-home"><span class="assistant-symbol">${icon("spark")}</span><p class="eyebrow">A LITTLE HELP GOES A LONG WAY</p><h2>What are we fixing?</h2><p>Turn a problem into a clear next step, a useful script or a reusable template.</p><div class="assistant-composer"><label>Operating system<select id="assistant-platform"><option value="windows">Windows · PowerShell</option><option value="linux">Linux · shell</option></select></label><label>Your task<textarea id="assistant-task" rows="4" placeholder="Write a script that checks disk space and reports services that failed to start"></textarea></label><div class="toolbar">${button("assistant-start", icon("spark") + " Ask Speck", true)}<small>${cfg.configured ? "Connected · " + esc(cfg.model) : "Connect OpenAI in Settings"}</small></div></div><div class="assistant-prompts">${["Diagnose a slow workstation", "Check DNS and connectivity", "Draft a software installer"].map((s) => `<button class="secondary" data-prompt="${esc(s)}">${esc(s)}</button>`).join("")}</div><p class="muted">AI drafts are reviewed before execution. For machine context, use Ask AI in its detail panel.</p></div>`,
    );
