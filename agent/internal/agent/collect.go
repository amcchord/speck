...
func Collect(foregroundDir string) map[string]any {
	...
	result["capabilities"] = map[string]any{"managed_operations": true, "automatic_updates": true, "screen_preview": true, "commands": true, "files": true, "powershell": hasPowerShell(), "active_app": result["active_app"] != nil, "desktop_note": "Browser desktop requires a local RDP/VNC service; headless Linux opens an interactive web shell through the agent"}
