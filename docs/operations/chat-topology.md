...
Settings → Slide Chat → Create integration token now offers optional Proxmox connection grants. Each checked connection discloses every host/guest in that connection; select only a client's own connections for a named-site token. The site's endpoint telemetry remains restricted to that exact site. An explicit All sites (`*`) token is available for full-network overviews and must be connected to All clients in Chat.
